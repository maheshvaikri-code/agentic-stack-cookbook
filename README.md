...
| Recipe | Scenario | Uses | Status |
|--------|----------|------|--------|
| [Audit bundle for one decision](recipes/88_audit_bundle_for_one_decision/) | one file that replays the exact context six months later, with nothing else on the machine | Contexel | ✅ in CI |
...
| [Where graph retrieval loses](recipes/79_where_graph_retrieval_loses/) | the corpus where our own approach is beaten by flat search, and the property that causes it | RudraDB | ✅ in CI |
